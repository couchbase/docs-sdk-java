...
A `ping` can be performed either at the `Cluster` or at the `Bucket` level. They are very similar, altough at the `Bucket` level also the Key/Value and View connections for the specific bucket are taken into account.
...
[source,java]
----
cluster.ping(pingOptions().serviceTypes(EnumSet.of(ServiceType.QUERY)));
----
...
System.err.println(cluster.diagnostics().exportToJson());
----
