...
The difference between the two is that if the document key already exists, the `insert` operation fails,
// throwing the `DocumentExistsException` error. However, the `upsert` operation succeeds, replacing the content.
