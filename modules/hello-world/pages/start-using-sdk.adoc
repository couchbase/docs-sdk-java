...
+
--
The Java SDK includes Capella’s standard Certificate Authority (CA) certificates by default, so you don't need any additional configuration.
...
SDK 3.4 introduced a `wan-development` Configuration Profile, which provides pre-configured timeout settings suitable for working in high latency environments.
