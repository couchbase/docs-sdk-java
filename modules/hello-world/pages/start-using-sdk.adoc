...
[source,java]
----
include::devguide:example$CloudConnect.java[tag=connect,indent=0]
----
--
...
For Capella, you will need to enable TLS.
From 3.3, the Java SDK includes Capella's standard certificates by default, so you do not need to additional configuration.
See xref:howtos:managing-connections.adoc[Managing Connections] for full details if you have more complex requirements.
