...
You will also need to enable TLS, but in this case using a `SecurityConfig`:
...
Its basic usage is shown in the example above, but if you want to learn more see xref:ref:client-settings.adoc#constrained-network-environments[Constrained Network Environments].
--
...
If you're developing client code on the same VM or machine as the Couchbase Server, you can just set the `connectionString` variable to equal `localhost`.
