...
=== Upgrading
IMPORTANT: All existing users are strongly recommended to read this section.
...
* https://issues.couchbase.com/browse/TXNJ-125[TXNJ-125]: Our transactions stage changes to a document alongside that document.
...
* https://issues.couchbase.com/browse/TXNJ-246[TXNJ-246]: Write-write conflict has been tuned to backoff-and-retry much less aggressively, which will result in much less 'churn' and overhead.
* https://issues.couchbase.com/browse/TXNJ-86[TXNJ-86]: T1 previously had to backoff and retry until either T2 completed, or cleanup discovered it (with default settings, that can take up to a minute).
Now, T1 can proceed to overwrite the document if it finds T2 has expired or completed.
* https://issues.couchbase.com/browse/TXNJ-217[TXNJ-217]: Performance improvement - T1 now only fetches the T2-specific data from T2's Active Transaction Record (ATR), rather than the full document.
...
* https://issues.couchbase.com/browse/TXNJ-208[TXNJ-208]: If cleanup is still unable to cleanup a transaction after two hours, it will begin logging that failure at WARN level.
...
* https://issues.couchbase.com/browse/TXNJ-13[TXNJ-13]: Performance improvement - cleanup has been tuned to remove unnecessary writes.
* https://issues.couchbase.com/browse/TXNJ-108[TXNJ-108]: Cancel cleanup if a pending transaction is discovered to now be committed.
This should not occur, and is simply a safety-check.
* https://issues.couchbase.com/browse/TXNJ-229[TXNJ-229]: Cleanup already checked that documents are in the expected state before cleaning them up, and now also uses CAS to verify that this is still the case when it gets to the cleanup point.
* https://issues.couchbase.com/browse/TXNJ-240[TXNJ-240]: Cleanup of documents is now done with durable writes.
* https://issues.couchbase.com/browse/TXNJ-232[TXNJ-232]: The expiry field inside an attempt now reflects time elapsed inside the transaction.

=== API Impacting
* https://issues.couchbase.com/browse/TXNJ-227[TXNJ-227]: A runtime check is now performed to ensure the minimum required java-client version is used.
...
* https://issues.couchbase.com/browse/TXNJ-219[TXNJ-219]: The `TransactionConfigBuilder` documentation stated that underlying Key-Value reads and writes would use kvTimeout and kvDurableTimeout from the configured environment if one is not specified.
That was not being done, and now is.

=== Bug Fixes
* https://issues.couchbase.com/browse/TXNJ-237[TXNJ-237]: A Scala bug with Class.getSimpleName meant the library could not be called from Scala.
* https://issues.couchbase.com/browse/TXNJ-250[TXNJ-250]: MAV reads should return post-transaction content on COMPLETED, not just COMMITTED.

=== Reliability Improvements
* https://issues.couchbase.com/browse/TXNJ-249[TXNJ-249]: The documentation advises applications to not catch exceptions inside the lambda (and if they have to, to propagate them).
...
* https://issues.couchbase.com/browse/TXNJ-231[TXNJ-231]: Rollback previously assumed that documents had not changed between them being staged and rolled back.
