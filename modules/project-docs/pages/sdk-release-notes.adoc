...
Remove Tracer argument from OpenTelemetrySpan.
...
Retry opening the bucket until timeout when it is not found, to allow it to created after.
* https://issues.couchbase.com/browse/JVMCBC-910[JVMCBC-910]:
WaitUntilReady needs to wait if bucket not present yet.
...
Support collections with BatchHelper.
...
Allow the Orphan Reporter to be disabled.
...
Escape the scope for scope-level queries.
* https://issues.couchbase.com/browse/JCBC-1746[JCBC-1746]:
Expose partition information from the query management API.
