...
Warn after 2 hours of failing to cleanup a transaction (previous setting was 2 days).
...
Do not wait for num (kv) nodesExt to equal number of nodes on Bucket waitUntilReady.
* https://issues.couchbase.com/browse/JVMCBC-1120[JVMCBC-1120]:
`ClusterConfig#allNodeAddresses` needs to take global config into account.
...
Allow for configurations with only TLS ports.
