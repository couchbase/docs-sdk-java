...
Rename EventingFunction validate_ssl_certificates to conform to spec.
* https://issues.couchbase.com/browse/JVMCBC-1020[JVMCBC-1020]:
If query request has target, honor it for prepare and execute, so they are both run on the same node. This fix adds `target` property to QueryRequest, and removes need for `TargetedQueryRequest`.
