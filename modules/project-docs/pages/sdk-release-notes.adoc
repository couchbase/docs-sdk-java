...
Adds a new API to `Collection`, `getOrNull`.  This behaves identically to the regular Key-Value get call except that if the document does not exist then null is returned rather than an exception being raised, avoiding the slight overhead of exception creation on JVM platforms.
