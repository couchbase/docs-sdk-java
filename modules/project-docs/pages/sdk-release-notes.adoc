...
Any send HTTP request will send a hostname if hostnames are used, rather than IP, leading to consistent hostname use across the system.
	...
	Assignee	Status	Development
Major	Improvement	JVMCBC-915	Load CID for collection only, not whole manifest	Michael Nitschinger	RESOLVED	1 commit
Major	Improvement	JVMCBC-916	Use hostname from config for HTTP host header
