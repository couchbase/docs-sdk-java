...
http://docs.couchbase.com/sdk-api/couchbase-java-client-3.0.6/[API Reference] |
...
Expiries are supplied as a `Duration`, and now any such duration that is less than 50 years, will be interpreted as being a relative timestamp from the current time.
E.g. `Duration.ofDays(5)` will expire in 5 days time.
(Previously, a `Duration` longer than 30 days would be interpreted as being an absolute, or epoch time.
In order to preserve compatibility for users that worked around this behaviour, any `Duration` longer than 50 years will continue to be interpreted as that same epoch time.)
...
Swallow redundant global loading exception.
* https://issues.couchbase.com/browse/JVMCBC-856[JVMCBC-856]:
A just-opened connection in pool must not be cleaned up.
* https://issues.couchbase.com/browse/JVMCBC-858[JVMCBC-858]:
Do not ignore channel writeAndFlush failures.
* https://issues.couchbase.com/browse/JVMCBC-862[JVMCBC-862]:
Race condition with node identifiers on bootstrap.
* https://issues.couchbase.com/browse/JVMCBC-863[JVMCBC-863]:
Bucket-Level ping report includes other buckets.
