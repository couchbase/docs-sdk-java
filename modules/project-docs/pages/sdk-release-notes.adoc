...
echo metrics-micrometer ; grep '<version>' $src/metrics-micrometer/pom.xml | head -2 | tail -1 ; grep '<micrometer.version>' $src/micrometer/pom.xml 
////
...
SDK calculates incorrect replica for buckets with >= 2 replicas
==== Improvements
* https://jira.issues.couchbase.com/browse/JVMCBC-1573[JVMCBC-1573]:
ExtParallelUnstaging leading to OOM when many concurrent transactions executed
