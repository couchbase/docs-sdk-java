...
Allow to customize the ConsoleLogger format.
* https://issues.couchbase.com/browse/JVMCBC-1093[JVMCBC-1093</a>] -
When a DNS SRV lookup fails due to a missing SRV record, the SDK used to log a scary warning with a stack trace.
The lookup failure is typically harmless, so the log message was downgraded to INFO level without a stack trace.
