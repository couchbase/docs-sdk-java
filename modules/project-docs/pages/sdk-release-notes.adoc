...
The exception hierarchy has been modified to align closer with the other SDKs.
...
Fix default collection support when collections enabled.
* http://issues.couchbase.com/browse/JVMCBC-689[JVMCBC-689]: 
Getting `DecodingFailedException` while performing sub_doc operations.
...
Make sure cas-based observe fails for concurrent modifications.
