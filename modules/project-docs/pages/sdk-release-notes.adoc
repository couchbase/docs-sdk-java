...
Make `MemcachedProtocol::decodeStatus` inlineable.
* https://issues.couchbase.com/browse/JVMCBC-1135[JVMCBC-1135]:
Move `Core#reconfiguration` off IO threads.
...
Support a 7.1 performance optimisation for transactions that reduces memory requirements in the clients.
...
Update metrics and tracing dependencies.

=== Bug Fixes
* https://issues.couchbase.com/browse/JVMCBC-1125[JVMCBC-1125]:
Fix a rare `CompletionException` seen from transactions when a very aggressive cleanup window is configured.
* https://issues.couchbase.com/browse/JVMCBC-1136[JVMCBC-1136]:
Remove verbose transactions cleanup debug "stop on" logging that was being logged at INFO level.
* https://issues.couchbase.com/browse/JCBC-1993[JCBC-1993]:
Fix a `NullPointerException` when there is no FTS index.
* https://issues.couchbase.com/browse/JCBC-1955[JCBC-1955]:
Fixes a bug where concurrent transactions could hang due to scheduler starvation.
