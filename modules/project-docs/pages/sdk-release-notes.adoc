...
The internal packaged `netty` dependency has been reduced to the minimum amount of artifacts needed, trimming the resulting jar size a bit.
...
The code for get projections had an import for unbundled jackson which prevent it from working in environment where no unbundled jackson is on the classpath.
