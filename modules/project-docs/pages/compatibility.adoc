...
In general, the JVM removes worries about underlying OS compatibility, 
...
It is not possible to determine if the SDK runs under Alpine Linux when it starts up, so SDK bootstrapping fails.

Because Alpine Linux does not have full `glibc` support,
we recommend that you use an alternative distribution if possible. 
In case this is not possible, there are workarounds that can be applied.
