...
It is not possible to determine if the SDK runs under Alpine Linux when it starts up, so the SDK crashes.

We recommend not running under Alpine Linux.
Use an alternative base image instead with full glibc support. 
