...
public class Durability extends ConnectionBase {
    ...
    protected void doWork() {
        ...
        // In the Java SDK you must specify a factor matching the number of replicas you
        // have configured
        // if you want "maximum" persistence or replication
        // Here we expect 3 replicas configured so we can wait for persistence on 4
        // nodes total, replication on 3 replicas.
        try {
            bucket.defaultCollection().upsert(key, doc,
                    UpsertOptions.upsertOptions().durability(PersistTo.FOUR, ReplicateTo.THREE));
        } catch (Exception e) { // if the durability cannot be met
            if (e instanceof ReplicaNotConfiguredException) {
                // this exception is a fail fast if not enough replicas are configured on the
                // bucket
                ...
                // this exception occurs if enough replica are configured on the bucket but
                // currently not enough are online
        ...
        try {
            bucket.defaultCollection().upsert(key, doc, UpsertOptions.upsertOptions()
                    .durability(PersistTo.TWO, ReplicateTo.ONE).timeout(Duration.ofSeconds(1)));
        } catch (DurabilityImpossibleException e) {
            // if the durability cannot be met (eg. if the cluster detected that the replica
            // wasn't available due to failover)
            LOGGER.error("Durability Exception", e);
        } catch (RuntimeException e) {
            System.out.println(e);
            if (e.getCause() instanceof TimeoutException) {
                // if one of the nodes isn't responsive, a TimeoutException rather than
                // DurabilityException may occur
