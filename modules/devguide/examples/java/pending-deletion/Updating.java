...
public class Updating extends ConnectionBase {
    ...
    protected void doWork() {
        ...
        // store the document (upsert will always work whether or not a value is already
        // associated to the key)
        MutationResult result = bucket.defaultCollection().upsert(key, content);
        LOGGER.info("Result after upsert: " + result); // notice the CAS changed (the returned document is updated with
                                                       // correct CAS)
        ...
        // on the other hand, updating works (it would have failed if the key was not in
        // database)
        result = bucket.defaultCollection().replace(key, content);
        LOGGER.info("Replaced the old document by the new one: " + result); // notice the document's CAS changed
                                                                            // again...

        LOGGER.info("Got the following from database: " + bucket.defaultCollection().get(key)); // ... which is
                                                                                                // consistent with a get
                                                                                                // (RYOW)
    }
