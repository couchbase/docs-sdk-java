...
+
From Couchbase 7.0, Out-of-Order returns prevent operations from being blocked by long-running KV requests, potentially improving performance for durable writes and multi-document ACID transactions.
...
Note, changing the setting will only effect Server versions 7.0 onwards.
