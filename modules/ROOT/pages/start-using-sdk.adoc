...
Information on new features, fixes, known issues as well as information on how to install older release versions is xref:relnotes-java-sdk.adoc[in the release notes].
...
NOTE: By default, from Couchbase Java SDK 2.6.0, [.a[i]`AlternateAddress` is implemented, for connecting to nodes in a NATed environment, such as Docker containers. An xref:client-settings.adoc[environmental setting] can be made, or the SDK will simply compare the bootstrap list, and make the right choice for addressing the alternate address in a multi-network configuration.
...
The reference for the latest version can be found http://docs.couchbase.com/sdk-api/couchbase-java-client-2.7.1/[here^].
