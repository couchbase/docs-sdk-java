...
:page-aliases: howtos:managing-connections,howtos:multi-network,connecting,connection-advanced,6.0@server:developer-guide:connection-advanced
...
All threads used by the SDK are daemon threads, so even if you do not `disconnect()` manually, your JVM will be able to exit.
