...
WARNING: You are strongly recommended against using Round Robin DNS, which can return a different address at each connection, while Java uses $host. 
Use of a xref:managing-connections.adoc#using-dns-srv-records[Service record (SRV record)], to provide real hostnames (A records) will ensure consistent connections.
