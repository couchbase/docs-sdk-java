...
Changed the default HTTP idle timeout to 4 seconds (from 30), to make sure that when running with Couchbase Server 6.x and later sockets are not closed by the server prematurely. This happens because for security reasons, the HTTP idle time has been reduced and the server will close those connections after 5 seconds.
