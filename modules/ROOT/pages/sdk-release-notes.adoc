...
Do not eagerly resolve network addresses when parsing the cluster configuration. This makes sure that a setup works where the client SDK runs outside of kubernetes and the cluster inside (utilizing the "alternate address" feature of the cluster)* http://issues.couchbase.com/browse/JVMCBC-677[JVMCBC-677]: Fixed a "division by 0" error in the core layer which only happened during shutdown and is perceived not to have an impact to actual operations.
