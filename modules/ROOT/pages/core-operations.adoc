...
You can atomically increment or decrement the numerical value of a special counter document.
...
In the above example, a counter is created by using the `counter` Python method with an `initial` value.
...
This enables you to work with counters using `get()` and `upsert()` operations -- allowing, _inter alia_, the use of durabelety options with the operations.
You will find several ways of working with counters https://docs.couchbase.com/sdk-api/couchbase-java-client-2.7.16/com/couchbase/client/java/Bucket.html#counter(java.lang.String,long)[in the API docs].
