...
.Event Bus Stability
====
While the event bus functionality itself is considered stable, the events itself maybe not. Please only consume the events you are interested in and add error handling code should they do not like you intended.
====
...
The following code subscribes to the event bus and prints out all events with INFO or WARN leven that are published on it:
