...
So in some cases the application will not need to supply a xref:concurrent-mutations-cluster.adoc[CAS] value to protect
...
In Couchbase Server 6.5 and up, this is built upon with Synchronous Replication, which uses the concept of majority to indicate the number of configured Data Service nodes to which commitment is required:
