...
The `ReplicaRead` feature, introduced in Server 7.6, allows Sub-Doc to work with replica copies of the data in the same way that our data service can work with replicas of full documents
(using `collection.get_any_replica` and `collection_get_all_replicas`).

`LookupInAnyReplica` will return the first response -- from active or replica:
...
While `LookupInAllReplicas` will fetch all replicas (and the active copy), and return all responses.
