...
because most production deployments at least monitor CPU and memory usage (i.e. through JMX).
...
Each metric contains tags that allow you to group them in different ways, including the service type (i.e. `query`) or the server hostname.
