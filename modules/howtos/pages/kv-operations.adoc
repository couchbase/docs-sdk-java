...
The value of a document can be increased or decreased atomically using `CounterResult increment()` and `CounterResult decrement()` -- more details in https://docs.couchbase.com/sdk-api/couchbase-java-client/com/couchbase/client/java/BinaryCollection.html[BinaryCollection] in the API guide.
////
.Increment
[source,csharp]
----
