...
To give an example of working with an absolute time stamp, rather than an offset, here is one way of setting the expiry time of the document to 60 days:
...
try {
    MutationResult result = collection.upsert("test", JsonObject.create().put("test", "testing"), 
    UpsertOptions.upsertOptions().expiry(Duration.ofMillis(time))); 
    System.out.println(result);
} catch (Exception e){ 
    e.printStackTrace(); 
}
----
// include::example$KvOperations.java[tag=expiry-abs]
