...
Here is an example showing an upsert in the `airports` collection, which lives in the `inventory` scope within `travel-sample` bucket:

[source,java]
----
include::example$ScopedKvOperations.java[tag=upsert]
----
