...
Couchbase's `Collection.get()` method returns a `GetResult` object, which can then be cast back into a `JsonObject`.
...
== Using Java's `Map` collection

The `JsonObject` is modelled on the classic Java `Map` collection, and in fact is backed by one.
...
We can now cast the result to Jackson's `JsonNode`, and use its rich set of methods to identify and handle arbitrary values:
