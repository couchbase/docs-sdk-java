...
For each service (i.e. Key/Value or Query) an entry exists in the outer JSON array. 
The top N (10 bydefault) slowest operations are collected and displayed, sorted by the total duration. 
This allows to get quick visibility into the "worst offenders" and continue troubleshooting.
...
Note that we still recommend to migrate eventually since OpenTracing has been sunset.
