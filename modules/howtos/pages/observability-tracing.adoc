...
In addition, users may want to consider using this https://github.com/couchbaselabs/couchbase-jvm-sdk-inmemory-tracer[in-memory tracer], which allows writing aggregated reports of all operations to the log, writing all operations to files in JSON format, and is flexible to allow the OpenTelemetry data to be processed however desired.
