...
NOTE: If developing on a single node, set replicas to 0 for your bucket (see xref:6.5@server:manage:manage-buckets/create-bucket.adoc#couchbase-bucket-settings[GUI] or xref:6.5@server:cli:cbcli/couchbase-cli-bucket-create.adoc#options[command line] bucket options), as the default durability requirement of 1 cannot be accomplished on just one node.
