...
see the xref:server:cli:cbcli/couchbase-cli-setting-alternate-address.adoc[reference] for further information.
...
Couchbase's next generation connection protocol, introduced in Java SDK 3.5 and xref:operator::concept-cloud-native-gateway.adoc[Couchbase Autonomous Operator 2.6.1] --
and available as a xref:cloud-native-gateway:intro:about-cng.adoc[standalone product] for use with self-managed Couchbase Server from 8.0 onwards --
can be enabled simply by changing the connection string to `couchbase2://` but there are a few differences to be aware of, described <<#limitations,below>>.
...
See the xref:concurrent-async-apis.adoc[] page for more information.
