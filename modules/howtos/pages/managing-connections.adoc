...
+
--
The Java SDK includes Capella’s standard root certificate by default, which means you don’t need any additional configuration to enable TLS.
Simply use `couchbases://` in your connection string, or `SecurityConfig.enableTls(true)` in your `SecurityConfig` settings.
--

Couchbase Server::
+
--
As of SDK 3.4, if your cluster's root certificate is signed by a well known Certificate Authority (e.g., GoDaddy, Verisign, etc) and is included in the JVM's trust store, you no longer need to configure the certificate in the `securityConfig` settings.

[source,java]
----
include::example$ManagingConnections.java[tag=tls-no-trust,indent=0]
----

However, you can still provide a certificate explicitly if necessary:
